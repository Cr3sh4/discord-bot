...
try {
    ...
    musicbot.playFunction = (msg, suffix, args, ignore) => {
      ...
      let vc = client.channels.cache.get(val => val.channel.guild.id == msg.member.guild.id)
      ...
      if (searchstring.startsWith("https://youtu.be/") || searchstring.startsWith("https://www.youtube.com/") && searchstring.indexOf("&") != -1) searchstring = searchstring.split("&")[0];
    ...
    musicbot.shuffleFunction = (msg, args) => {
      let q = musicbot.getQueue(msg.guild.id);
      ...
      {
        msg.channel.send(musicbot.note('fail', `You're not in a voice channel.`));
        return;
      }
      if (!musicbot.queues.has(msg.guild.id)) return msg.channel.send(musicbot.note('fail', `No queue for this server found!`));
      const voiceConnection = client.voice.connections.find(val => val.channel.guild.id == msg.guild.id);
